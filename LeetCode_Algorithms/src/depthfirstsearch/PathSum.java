...
/**
 * #112 Given a binary tree and a sum, determine if the tree has a root-to-leaf
...
public class PathSum {
	...
	public boolean dfs(TreeNode node, int targetSum, int currentSum) {
		...
		return (node.left == null ? false : dfs(node.left, targetSum, currentSum + node.val))
				|| (node.right == null ? false : dfs(node.right, targetSum, currentSum + node.val));
	}
